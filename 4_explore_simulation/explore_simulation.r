...
available.files <- list.files(path = "./inputs", pattern = ".csv",
  full.names = TRUE)
...
for (j in 1:n.sims) {
  ...
  mom.rows <- which(reg$male == 0 & reg$age.today >= 40 &
    reg$patrilineal == "1")
  ...
  parity1.was <- c(sum(substr(d$pcode, 1, 1) == "0"),
    sum(substr(d$pcode, 1, 1) == "1"))

  tar <- which(d$n.kids >= 2)
  parity2.was <- c(
  sum(as.numeric(lapply(strsplit(substr(d$pcode[tar], 1, 2), ""),
    function(z) sum(as.numeric(z)) == 0))),
  sum(as.numeric(lapply(strsplit(substr(d$pcode[tar], 1, 2), ""),
    function(z) sum(as.numeric(z)) == 1))),
  sum(as.numeric(lapply(strsplit(substr(d$pcode[tar], 1, 2), ""),
    function(z) sum(as.numeric(z)) == 2))))

  tar <- which(d$n.kids >= 3)
  parity3.was <- c(
  sum(as.numeric(lapply(strsplit(substr(d$pcode[tar], 1, 3), ""),
    function(z) sum(as.numeric(z)) == 0))),
  sum(as.numeric(lapply(strsplit(substr(d$pcode[tar], 1, 3), ""),
    function(z) sum(as.numeric(z)) == 1))),
  sum(as.numeric(lapply(strsplit(substr(d$pcode[tar], 1, 3), ""),
    function(z) sum(as.numeric(z)) == 2))),
  sum(as.numeric(lapply(strsplit(substr(d$pcode[tar], 1, 3), ""),
    function(z) sum(as.numeric(z)) == 3))))

  tar <- which(d$n.kids >= 4)
  parity4.was <- c(
  sum(as.numeric(lapply(strsplit(substr(d$pcode[tar], 1, 4), ""),
    function(z) sum(as.numeric(z)) == 0))),
  sum(as.numeric(lapply(strsplit(substr(d$pcode[tar], 1, 4), ""),
    function(z) sum(as.numeric(z)) == 1))),
  sum(as.numeric(lapply(strsplit(substr(d$pcode[tar], 1, 4), ""),
  ...
  sum(as.numeric(lapply(strsplit(substr(d$pcode[tar], 1, 4), ""),
    function(z) sum(as.numeric(z)) == 3))),
  sum(as.numeric(lapply(strsplit(substr(d$pcode[tar], 1, 4), ""),
    function(z) sum(as.numeric(z)) == 4))))

  tar <- which(d$n.kids >= 5)
  parity5.was <- c(
  sum(as.numeric(lapply(strsplit(substr(d$pcode[tar], 1, 5), ""),
    function(z) sum(as.numeric(z)) == 0))),
  sum(as.numeric(lapply(strsplit(substr(d$pcode[tar], 1, 5), ""),
    function(z) sum(as.numeric(z)) == 1))),
  sum(as.numeric(lapply(strsplit(substr(d$pcode[tar], 1, 5), ""),
...
parity3.pr <- round(
    100 - round(colMeans(parity3.pr.tab.pat, na.rm = TRUE), 1)
  , 1)
...
hex(-sqrt(3) * c(-1.5, -0.5, 0.5, 1.5), rep(-4.5, 4),
  col = parity3.cols, border = "black")
...
text(-sqrt(3) * c(-1.5, -0.5, 0.5, 1.5), rep(-4.5, 4), parity3.pr.txt,
  col = "black")
...
right.end <- 1.5
line.height <- (-6.3)
points(seq(right.end, left.end, length.out = n), rep(line.height + (.1), n),
  col = color.list, pch = 20)
...
my.x0 <- 1.69
my.y0 <- 0.09
arrows(x0 = my.x0, y0 = my.y0, x1 = my.x0 + sqrt(3) / 2, y1 = my.y0 - 3 / 2,
  length = 0.2, angle = 15)
arrows(x0 = -my.x0, y0 = my.y0, x1 = -my.x0 - sqrt(3) / 2, y1 = my.y0 - 3 / 2,
  length = 0.2, angle = 15)
...
parity3.pr <- round(100 - round(colMeans(parity3.pr.tab.mat,
  na.rm = TRUE), 1), 1)
...
my.cols <- names(gradient_maker(0, 1, cols = c("darkorange2", "steelblue"),
  n = 8))
...
text(-sqrt(3) * c(-0.5, 0.5), rep(-1.5, 2), parity1.pr.txt, col = "black")
text(-sqrt(3) * c(-1, 0, 1), rep(-3, 3), parity2.pr.txt, col = "black")
text(-sqrt(3) * c(-1.5, -0.5, 0.5, 1.5), rep(-4.5, 4),
  parity3.pr.txt, col = "black")
