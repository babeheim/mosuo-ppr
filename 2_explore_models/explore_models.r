...
# explore parity 1 model

load("./inputs/parity1_re.robj")

## prepare table of marginal estimates from model
...
polygon(c(5.7, 5.7, 6.3, 6.3), c(0, pr.1P.mean, pr.1P.mean, 0),
  col = "sandybrown", border = NA)
points(c(1, 2, 5, 6), c(pr.0M.mean, pr.1M.mean, pr.0P.mean, pr.1P.mean),
  pch = 20)
...
## combine tables from parity 1 and parity 2 models, and export
...
plot(1:8, ylim = c(0, 0.05), ylab = "Annual Probability of Birth",
  xlab = "Sex Combination at Parity 2", xaxt = "n", col = NULL,
  xlim = c(0, 9), frame.plot = FALSE, las = 1)
abline(h = 0:10 / 10, col = gray(0.8))
...
axis(1, at = c(1, 2, 3, 6, 7, 8), labels =
  c("D/D", "D/S", "S/S", "D/D", "D/S", "S/S"))
...
# explore parity 2 alternative model 1

load("./inputs/parity2_pooled1_re.robj")
...
rownames(output) <- gsub("a_sigma", "Mother Random Effect", rownames(output))
rownames(output) <- gsub("b2", "Patrilineal", rownames(output))
rownames(output) <- gsub("b3", "No Lineal Children x Patrilineal",
  rownames(output))
rownames(output) <- gsub("b4", "Age 15-19", rownames(output))
...
# explore parity 2 alternative model 2

load("./inputs/parity2_pooled2_re.robj")
...
rownames(output) <- gsub("a_sigma", "Mother Random Effect", rownames(output))
rownames(output) <- gsub("b2", "Patrilineal", rownames(output))
rownames(output) <- gsub("b3", "Balanced Sex Ratio x Patrilineal",
  rownames(output))
rownames(output) <- gsub("b4", "Age 15-19", rownames(output))
rownames(output) <- gsub("b5", "Age 20-24", rownames(output))
rownames(output) <- gsub("b6", "Cohort 1930-1939", rownames(output))
...
## combine tables from parity 2 alternative models, and export
...
plot(1:6, ylim = c(0, .05), ylab = "Annual Probability of Birth",
  xlab = "Parity 2 Sibset", xaxt = "n", col = NULL, xlim = c(0, 7),
  frame.plot = FALSE, las = 1)
...
polygon(c(5.7, 5.7, 6.3, 6.3), c(0, pr.nolineal.pat.mean, pr.nolineal.pat.mean, 0),
  col = "sandybrown", border = NA)
points(c(1, 2, 5, 6), c(pr.lineal.mat.mean, pr.nolineal.mat.mean,
  pr.lineal.pat.mean, pr.nolineal.pat.mean), pch = 20)
...
plot(1:6, ylim = c(0, 0.05), xlim = c(0, 7),
  ylab = "Annual Probability of Birth", xlab = "Parity 2 Sibset",
  xaxt = "n", col = NULL, frame.plot = FALSE, las = 1)
abline(h = 0:10 / 20, col = gray(0.8))
...
polygon(c(5.7, 5.7, 6.3, 6.3), c(0, pr.mixed.pat.mean, pr.mixed.pat.mean, 0),
  col = "sandybrown", border = NA)
points(c(1, 2, 5, 6), c(pr.homog.mat.mean, pr.mixed.mat.mean,
  pr.homog.pat.mean, pr.mixed.pat.mean), pch = 20)
