...
model2re <- alist(
    ...
    conc ~ dbinom(1, p),
    logit(p) <- a[id] + b1*son + b2*patrilineal + b3*son_patrilineal 
    + b4*age15 + b6*yearbirth1930 + b7*yearbirth1940 + b8*yearbirth1950 
    + b9*yearbirth1970 + b10*yearbirth1920 + b11*yearbirth1980 + b12*mi_job 
    + b13*highestgrade + b14*bly + b15*age25 + b16*age30 + b17*age35 + b18*age40,
...
start.time <- Sys.time()
m2re <- map2stan(model2re, data=d2, sample=enable_sampling)
save(m2re, file='./temp/parity1_re.robj')
stop.time <- Sys.time()

cat(task_timer('fit parity1 random effects model'), file='./temp/fitmodels_log.txt')

# analysis of parity 2
...
model3 <- alist(
    ...
    conc ~ dbinom(1, p),
    logit(p) <- a[id] + b1*pcode10 + b2*pcode11 + b3*patrilineal 
    + b4*pat_10 + b5*pat_11 + b6*age15 + b7*age20 + b8*yearbirth1930 
    + b9*yearbirth1940 + b10*yearbirth1950 + b11*yearbirth1970 
    + b12*yearbirth1920 + b14*mi_job + b15*highestgrade + b16*bly 
...
start.time <- Sys.time()
m3re <- map2stan(model3, data=d3, iter=4000, sample=enable_sampling)
save(m3re, file='./temp/parity2_re.robj')
stop.time <- Sys.time()

cat(task_timer('fit parity2 random effects model'), 
	file='./temp/fitmodels_log.txt', append=TRUE)
...
model4 <- alist(
    ...
    conc ~ dbinom(1, p),
    logit(p) <- a[id] + b1*type_nolineal + b2*patrilineal + b3*no_lineal_pat + 
    b4*age15 + b5*age20 + b6*yearbirth1930 + b7*yearbirth1940 
    + b8*yearbirth1950 + b9*yearbirth1970 + b10*yearbirth1920 
    + b12*mi_job + b13*highestgrade + b14*bly + b15*age30 + b16*age35 + b17*age40,
    a[id] ~ dnorm(a_mu, a_sigma),
...
start.time <- Sys.time()
m4re <- map2stan(model4, data=d4, sample=enable_sampling)
save(m4re, file='./temp/parity2_pooled1_re.robj')
stop.time <- Sys.time()

cat(task_timer('fit parity2 "lineal" random effects model'), 
	file='./temp/fitmodels_log.txt', append=TRUE)
...
model5 <- alist(
    ...
    conc ~ dbinom(1, p),
    logit(p) <- a[id] + b1*type_mixed + b2*patrilineal + b3*Mpat + 
    b4*age15 + b5*age20 + b6*yearbirth1930 + b7*yearbirth1940 + 
    b8*yearbirth1950 + b9*yearbirth1970 + b10*yearbirth1920 + 
    b12*mi_job + b13*highestgrade + b14*bly + b15*age30 + b16*age35 + b17*age40,
    a[id] ~ dnorm(a_mu, a_sigma),
...
start.time <- Sys.time()
m5re <- map2stan(model5, data=d5, sample=enable_sampling)
save(m5re, file='./temp/parity2_pooled2_re.robj')
stop.time <- Sys.time()

cat(task_timer('fit parity2 "balanced" random effects model'), 
	file='./temp/fitmodels_log.txt', append=TRUE)
...
model6re <- alist(
    ...
    conc ~ dbinom(1, p),
    logit(p) <- a[id] + b2*patrilineal + b4*age15 + b6*yearbirth1930 
    + b7*yearbirth1940 + b8*yearbirth1950 + b9*yearbirth1970 + b10*yearbirth1920 
    + b11*yearbirth1980 + b12*mi_job + b13*highestgrade + b15*age25 + b16*age30 
    + b17*age35 + b18*age40,
    a[id] ~ dnorm(a_mu, a_sigma),
    c(a_mu, b2, b4, b6, b7, b8, b9, b10, b11, b12, b13, b15, b16, b17, b18) ~ dnorm(0, 100),
    a_sigma ~ dcauchy(0,1)
    
    )
    
start.time <- Sys.time()
m6re <- map2stan(model6re, data=d6, sample=enable_sampling)
save(m6re, file='./temp/parity0_re.robj')
stop.time <- Sys.time()

cat(task_timer('fit parity0 random effects model'), 
	file='./temp/fitmodels_log.txt', append=TRUE)
...
model7 <- alist(
    ...
    conc ~ dbinom(1, p),
    logit(p) <- a[id] + b1*pcode100 + b2*pcode110 + b3*pcode111 + b4*patrilineal
     + b5*pat_100 + b6*pat_110 + b7*pat_111 + b9*age20 + b10*yearbirth1930
     + b11*yearbirth1940 + b12*yearbirth1950 + b14*yearbirth1920 + b16*highestgrade
     + b17*bly + b18*age30 + b19*age35 + b20*age40,
    a[id] ~ dnorm(a_mu, a_sigma),
...
start.time <- Sys.time()
m7re <- map2stan(model7, data=d7, iter=4000, sample=enable_sampling)
save(m7re, file='./temp/parity3_re.robj')
stop.time <- Sys.time()

cat(task_timer('fit parity3 random effects model'), 
	file='./temp/fitmodels_log.txt', append=TRUE)
...
model8 <- alist(
    ...
    conc ~ dbinom(1, p),
    logit(p) <- a[id] + b1*pcode1000 + b2*pcode1100 + b3*pcode1110 
    + b4*pcode1111 + b5*patrilineal + b6*pat_1000 + b7*pat_1100 + b8*pat_1110 
    + b9*pat_1111 + b10*yearbirth1920 + b11*yearbirth1930 + b12*yearbirth1950 
    + b13*highestgrade + b14*bly + b15*age25 + b16*age30 + b17*age35,
    a[id] ~ dnorm(a_mu, a_sigma),
...
start.time <- Sys.time()
m8re <- map2stan(model8, data=d8, iter=4000, sample=enable_sampling)
save(m8re, file='./temp/parity4_re.robj')
stop.time <- Sys.time()

cat(task_timer('fit parity4 random effects model'), 
	file='./temp/fitmodels_log.txt', append=TRUE)
